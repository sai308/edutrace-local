...
const { students: enrichedStudents, loadExamData, meets, tasks, groupsMap } = useExamData();
...
async function loadData() {
    ...
    try {
        ...
        const allAssessments = await repository.getAllFinalAssessments();

        // Get students for this specific group
        const groupStudents = await repository.getMembersByGroup(props.group.name);
...
// Computed filtered and sorted assessments
    ...
    result.sort((a, b) => {
        ...
        switch (sortColumn.value) {
            ...
            case 'grade':
                ...
                break;
            case 'assessmentType':
                valA = a.assessmentType;
                valB = b.assessmentType;
                break;
            case 'createdAt':
                ...
                break;
            case 'syncedAt':
                valA = a.syncedAt ? new Date(a.syncedAt).getTime() : 0;
                valB = b.syncedAt ? new Date(b.syncedAt).getTime() : 0;
                break;
            case 'documentedAt':
                valA = a.documentedAt ? new Date(a.documentedAt).getTime() : 0;
                valB = b.documentedAt ? new Date(b.documentedAt).getTime() : 0;
                break;
            default:
...
async function toggleSynced(assessment) {
    ...
    await repository.updateAssessmentSyncStatus(assessment.id, newSyncedAt);
...
async function toggleDocumented(assessment) {
    ...
    await repository.updateAssessmentDocumentStatus(assessment.id, newDocumentedAt);
