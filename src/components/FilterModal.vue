...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm !-mt-6 p-4 animate-in fade-in duration-200">
    <div class="bg-card w-full max-w-2xl rounded-lg shadow-lg border flex flex-col h-[80vh] animate-in zoom-in-95 duration-200">
      
      <!-- Header -->
      <div class="p-4 border-b flex items-center justify-between">
        <h3 class="text-lg font-bold">Manage Teachers</h3>
      ...
      <div class="flex-1 overflow-y-auto min-h-[200px] space-y-4 p-4">
          ...
          <div v-if="sortedSelected.length > 0">
            <h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Selected</h4>
            <div class="space-y-1">
              <div 
          ...
          <div v-if="sortedAvailable.length > 0">
            <h4 class="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Available</h4>
            <div class="space-y-1">
              <div 
                ...
                @click="toggleUser(user)"
              >
                <div class="w-4 h-4 rounded border border-muted-foreground flex items-center justify-center transition-colors"></div>
          ...
          <!-- Empty State -->
          <div v-if="sortedSelected.length === 0 && sortedAvailable.length === 0" class="text-center py-8 text-muted-foreground text-sm">
      ...
      <div class="p-4 border-t bg-muted/10 flex justify-between items-center">
        <span class="text-sm text-muted-foreground">
          {{ selectedItems.size }} {{ mode === 'teachers' ? 'teachers' : 'participants ignored' }}
        </span>
        <div class="flex gap-2">
          <button 
